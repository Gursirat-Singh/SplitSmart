...
class BalanceService {
    ...
    `;
        ...
        const makeKey = (u1, u2, c) => `${u1}_${u2}_${c}`;
        const debts = new Map();
        ...
        const pairs = new Set(); // Format: userA|userB|currency (where userA < userB)
        const addPair = (u1, u2, c) => {
            const a = u1 < u2 ? u1 : u2;
            const b = u1 < u2 ? u2 : u1;
            pairs.add(`${a}|${b}|${c}`);
        };
        debtsRaw.forEach((d) => addPair(d.debtor_id, d.creditor_id, d.currency));
        settlementsRaw.forEach((s) => addPair(s.payer_id, s.payee_id, s.currency));
        const finalBalances = [];
        // 3. Compute Net Balances
        for (const pairStr of pairs) {
            const [userA, userB, currency] = pairStr.split('|');
